...
class DogReassignmentSystem:
    ...
    def load_distance_matrix(self, csv_url):
        ...
        try:
            ...
            # Debug: Show raw CSV structure
            st.info(f"🔍 CSV Debug - Shape: {df.shape}")
            st.info(f"🔍 CSV Debug - First row: {df.iloc[0].tolist()[:10]}")
            st.info(f"🔍 CSV Debug - First column: {df.iloc[:5, 0].tolist()}")
            
            # Find the actual header row with Dog IDs
            header_row_idx = 0
            
            # Check if first row has Dog IDs (should contain 'x' suffix)
            first_row_values = df.iloc[0, 1:].tolist()
            has_dog_ids = any(str(val).endswith('x') for val in first_row_values if not pd.isna(val))
            
            if not has_dog_ids:
                # Maybe the header is in row 1 instead of row 0
                if len(df) > 1:
                    second_row_values = df.iloc[1, 1:].tolist()
                    if any(str(val).endswith('x') for val in second_row_values if not pd.isna(val)):
                        header_row_idx = 1
                        st.info("🔍 Found Dog IDs in row 1 instead of row 0")
            
            # Extract Dog IDs from header row
            dog_ids = df.iloc[header_row_idx, 1:].tolist()  # Skip first cell
            ...
            for dog_id in dog_ids:
                ...
                # Only include if it looks like a Dog ID (contains 'x')
                if 'x' in clean_id:
                    clean_dog_ids.append(clean_id)
            
            st.info(f"🔍 Found {len(clean_dog_ids)} Dog IDs in header: {clean_dog_ids[:10]}")
            
            # Process data rows (start after header row)
            data_start_row = header_row_idx + 1
            
            for i in range(data_start_row, len(df)):
                row_dog_id = df.iloc[i, 0]  # First column
                ...
                # Only process if this looks like a Dog ID
                if 'x' not in row_dog_id:
                    continue
                ...
                for j, col_dog_id in enumerate(clean_dog_ids):
                    # Get distance value from corresponding cell
                    distance_val = df.iloc[i, j + 1]  # +1 to skip first column
            ...
            if len(self.distance_matrix) > 0:
                ...
                st.info(f"📏 CORRECTED Sample distances for {first_dog}: {sample_distances}")
